...
class PublishPacket:
    ...
    retain: bool
    topic: str
    packet_id: Optional[int]  # None for QoS 0
    payload: bytes
...
class SubscribePacket:
    ...
    packet_id: int
    topics: List[Tuple[str, int]]  # List of (topic_filter, requested_qos)
...
def parse_fixed_header(data: bytes) -> Tuple[PacketType, int, int, int]:
    ...
    remaining_length, length_bytes = decode_remaining_length(data, 1)
...
def parse_connect(data: bytes) -> ConnectPacket:
    ...
    # Protocol Name
    protocol_name, consumed = decode_string(data, offset)
    offset += consumed
    ...
    # Payload
    # Client Identifier
    client_id, consumed = decode_string(data, offset)
    offset += consumed
    ...
    will_message = None
    if flags.will_flag:
        will_topic, consumed = decode_string(data, offset)
        offset += consumed
        will_message, consumed = decode_binary(data, offset)
        offset += consumed
    
    # Username
    username = None
    if flags.username_flag:
        username, consumed = decode_string(data, offset)
        offset += consumed
    
    # Password
    password = None
    if flags.password_flag:
        password, consumed = decode_binary(data, offset)
        offset += consumed
...
def build_connack(session_present: bool, return_code: ConnectReturnCode) -> bytes:
    """Build CONNACK packet."""
    flags = 0x01 if session_present else 0x00
    payload = bytes([flags, return_code])
    return bytes([PacketType.CONNACK << 4]) + encode_remaining_length(len(payload)) + payload


def parse_publish(flags: int, data: bytes) -> PublishPacket:
    ...
    # Topic Name
    topic, consumed = decode_string(data, offset)
    offset += consumed
...
def build_publish(topic: str, payload: bytes, qos: int = 0, retain: bool = False, 
    ...
    packet_payload = variable_header + payload
    return bytes([(PacketType.PUBLISH << 4) | flags]) + encode_remaining_length(len(packet_payload)) + packet_payload


def build_puback(packet_id: int) -> bytes:
    """Build PUBACK packet."""
    payload = struct.pack("!H", packet_id)
    return bytes([PacketType.PUBACK << 4]) + encode_remaining_length(len(payload)) + payload


def build_pubrec(packet_id: int) -> bytes:
    """Build PUBREC packet."""
    payload = struct.pack("!H", packet_id)
    return bytes([PacketType.PUBREC << 4]) + encode_remaining_length(len(payload)) + payload


def build_pubrel(packet_id: int) -> bytes:
    """Build PUBREL packet (flags must be 0x02)."""
    payload = struct.pack("!H", packet_id)
    return bytes([(PacketType.PUBREL << 4) | 0x02]) + encode_remaining_length(len(payload)) + payload


def build_pubcomp(packet_id: int) -> bytes:
    """Build PUBCOMP packet."""
    payload = struct.pack("!H", packet_id)
    return bytes([PacketType.PUBCOMP << 4]) + encode_remaining_length(len(payload)) + payload


def parse_packet_id(data: bytes) -> int:
    """Parse packet ID from PUBACK/PUBREC/PUBREL/PUBCOMP."""
    if len(data) < 2:
        raise MalformedPacketError("Missing packet identifier")
    return struct.unpack("!H", data[0:2])[0]


def parse_subscribe(data: bytes) -> SubscribePacket:
    ...
    while offset < len(data):
        # Topic filter
        topic_filter, consumed = decode_string(data, offset)
        offset += consumed
...
def build_suback(packet_id: int, return_codes: List[int]) -> bytes:
    ...
    return bytes([PacketType.SUBACK << 4]) + encode_remaining_length(len(payload)) + payload


def parse_unsubscribe(data: bytes) -> UnsubscribePacket:
...
def build_unsuback(packet_id: int) -> bytes:
    ...
    payload = struct.pack("!H", packet_id)
    return bytes([PacketType.UNSUBACK << 4]) + encode_remaining_length(len(payload)) + payload
